...
class Player < ApplicationRecord
  ...
  validates :cash_in_hand, presence: true,
  ...
  scope :ordered_by_profit, -> { order("(cash_in_hand - total_loan) DESC") }
  scope :active, -> { where("player_results_count > 0") }

  # Calculate loan needed for a game
  def loan_needed_for_game(buy_in_amount)
    return 0 if cash_in_hand >= buy_in_amount
    buy_in_amount - cash_in_hand
  end

  # Calculate current profit/loss
  def current_profit
    cash_in_hand - total_loan
  end
  ...
  def participate_in_game(game_session, buy_in_amount)
    ...
    if loan_amount > 0
      update!(
        ...
        cash_in_hand: 0 # Since all cash is used before loan
      )
    else
      update!(cash_in_hand: cash_in_hand - buy_in_amount)
    end
  end
